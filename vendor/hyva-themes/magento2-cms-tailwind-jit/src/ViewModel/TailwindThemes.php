...
class TailwindThemes implements ArgumentInterface, OptionSourceInterface
{
    ...
    private function getAllTailwindThemes(): array
    {
        if (!$this->allTailwindThemes) {
            // Allow use with theme-module before the HyvaThemes service was added (< 1.3.18)
            $filter = class_exists(HyvaThemes::class)
                ? [ObjectManager::getInstance()->get(HyvaThemes::class), 'isHyvaThemeCode']
                : [$this, 'isDescendantOfHyvaReset'];
            $this->allTailwindThemes = filter($this->getAllThemes(), $filter);
        }
